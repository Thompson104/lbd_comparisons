...
from transformations.transformations import quaternion_disp
...
if IS_LAST_MOTION:
    ...
    if IS_FILTERED:
        ...
        '''
        JS_FILE_NAME = "js_crappy_motion_0_rev_mod1.csv"
        MOTION_FILE_NAME = "crappy_motion_0_rev.csv"
        PARAM_FILE_NAME = "crappy_motion_0_rev_mod1.csv"
        ERR_FILE_NAME = "err_crappy_motion_0_rev_mod1.csv"
...
#           csv_pos_fake.append([float(item)+0.06 if idx==2 else float(item) for idx, item in enumerate(tmp_pos_list)])
    ...
    return np.array(csv_times), csv_pos, csv_quat, csv_pos_fake
...
def main():
    time_stamp, csv_pos, csv_quat, csv_pos_fake = load_motion_data(global_dir=MOTION_FILE_NAME)
