...
objects = ('shoulder_pan', 'shoulder_lift', 'elbow', 'wrist_1', 'wrist_2', 'wrist_3')
...
class Window(QWidget):
	...
	bx = self.figure_2.add_subplot(111)

        # plot data, bar plot
        #ax.plot(data, '*-')
	ax.bar([i for i in range(2)], data, align='center', width=0.5, alpha=0.7)
	bx.bar([i for i in range(6)], js_state, color='r', align='center', alpha=0.7)

	# discards the old graph
        ax.hold(False)
	bx.hold(False)
	
	ax.set_xlabel("Pos Obj                                                             Orien Obj")
	#ax.legend((rects1[0], rects2[0]), ('Pos Obj', 'Orien Obj'))
